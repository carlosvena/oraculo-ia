    ...
    return OraculoScaffold(
      body: ListView(
        children: [
          ...
          const Divider(),
          const SizedBox(height: 12),
          Text('Empezar de nuevo', style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 8),
          const Text(
            'Borra todo el progreso local (misiones, XP, favoritos, historial) '
            'y vuelve al principio, como recién instalada. No se puede deshacer '
            '— hacé un respaldo antes si querés conservarlo.',
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            style: OutlinedButton.styleFrom(foregroundColor: Theme.of(context).colorScheme.error),
            icon: const Icon(Icons.restart_alt),
            label: const Text('VOLVER AL PRINCIPIO'),
            onPressed: () async {
              final confirmed = await showDialog<bool>(
                context: context,
                builder: (dialogContext) => AlertDialog(
                  title: const Text('¿Volver al principio?'),
                  content: const Text(
                    'Se va a borrar todo tu progreso guardado en este teléfono. '
              ...
              if (confirmed == true) {
                await ref.read(learningStateProvider.notifier).reset();
                if (context.mounted) {
                  setState(() => message = 'Progreso reiniciado. Volviste al principio.');
                }
              }
            },
          ),
